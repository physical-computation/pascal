...
BroadcastOutput pascal_tensor_broadcast_linalg(Tensor a, Tensor b, index_t out_shape[], index_t out_ndim) {
	pascal_tensor_assert((out_ndim == 1 || out_ndim == 2), "out_ndim must be 1 or 2");
	...
	if (a->ndim >= b->ndim) {
		...
		ndim  = a->ndim - 2 + out_ndim;
	} else {
		shape = malloc(sizeof(index_t) * b->ndim);
		ndim  = b->ndim - 2 + out_ndim;
	}

	index_t* a_stride = malloc(sizeof(index_t) * (ndim - out_ndim));
	index_t* b_stride = malloc(sizeof(index_t) * (ndim - out_ndim));

	for (int i = 0; i < out_ndim; i++) {
		shape[ndim - 1 - i] = out_shape[out_ndim - 1 - i];
	}

	int a_counter = a->ndim - 1 - out_ndim;
	int b_counter = b->ndim - 1 - out_ndim;
	int s_counter = ndim - 1 - out_ndim;
