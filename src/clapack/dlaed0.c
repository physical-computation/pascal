...
/* Subroutine */ int dlaed0_(integer* icompq, integer* qsiz, integer* n,
	...
	extern /* Subroutine */ int dlaed7_(integer*, integer*, integer*,
	                                    ...
	                                    integer*, integer*, doublereal*, integer*, doublereal*,
	                                    integer*, integer*, integer*, integer*, integer*, doublereal*, doublereal*, integer*, integer*);
	integer                     tlvls;
...
L10:
	...
	for (i__ = 0; i__ <= i__1; ++i__) {
		...
		if (*icompq == 2) {
			...
			if (*icompq == 1) {
				dgemm_("N", "N", qsiz, &matsiz, &matsiz, &c_b23, &q[submat * q_dim1 + 1], ldq, &work[iq - 1 + iwork[iqptr + curr]],
		...
		for (i__ = 0; i__ <= i__1; i__ += 2) {
			...
			if (*icompq == 2) {
				dlaed1_(&matsiz, &d__[submat], &q[submat + submat * q_dim1],
				        ldq, &iwork[indxq + submat], &e[submat + msd2 - 1], &msd2, &work[1], &iwork[subpbs + 1], info);
			} else {
