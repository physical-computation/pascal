...
extern double f__cabs(double, double);
		...
		#ifndef Pre20000310
			#ifdef Comment
Some versions of gcc, such as 2.95.3 and 3.0.4, are buggy under - O2 or - O3 : on IA32(Intel 80x87) systems, they may do comparisons on values computed in extended - precision registers.This can lead to the test "s > s0" that was used below being carried out incorrectly.The fix below cannot be spoiled by overzealous optimization, since the compiler cannot know whether gcc_bug_bypass_diff_F2C will be nonzero.(We expect it always to be zero.The weird name is unlikely to collide with anything.)
		...
		a     = (.1099557428756427618354862829619, .9857360542953131909982289471372)
                ...
                An alternative to the fix below would be to use 53 -
            bit rounding precision,
	...
	if (t > .01)
		...
		log(1 + x) = x - x ^ 2 / 2 + x ^ 3 / 3 - x ^ 4 / 4 + -...
				...
				= x(1 - x / 2 + x ^ 2 / 3 - +...)
		          ...
		          [sqrt(y ^ 2 + z ^ 2) - 1] *
		      [sqrt(y ^ 2 + z ^ 2) + 1] = y ^ 2 + z ^ 2 - 1,
						so

										sqrt(y ^ 2 + z ^ 2) -
								1 = (y ^ 2 + z ^ 2 - 1) / [sqrt(y ^ 2 + z ^ 2) + 1]
